...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function initFilters() {
        ...
        // ИСПРАВЛЕНИЕ УМНЫЙ РЕНДЕР: обновляем HTML только если он реально изменился
        if(sideFilterCategories && sideFilterCategories.innerHTML !== sideHtml) sideFilterCategories.innerHTML = sideHtml;
        if(mobFilterCategories && mobFilterCategories.innerHTML !== mobHtml) mobFilterCategories.innerHTML = mobHtml;
    ...
    function renderBatch() {
        ...
        // УБРАНО: grid.innerHTML = ''; <-- Это вызывало основное мигание
        ...
        // ИСПРАВЛЕНИЕ УМНЫЙ РЕНДЕР:
        // Сначала собираем весь HTML, а затем обновляем только если есть изменения
        let newHtml = '';
        if (nextBatch.length > 0) {
            newHtml = nextBatch.map(p => window.renderProductCard ? window.renderProductCard(p) : '').join('');
        }
    ...
    const refreshFromSync = (event) => {
        ...
        // ИСПРАВЛЕНИЕ: увеличен таймаут с 80 до 150 для надежного дебаунса
...
function renderCatalog(items) {
    ...
    // ИСПРАВЛЕНИЕ УМНЫЙ РЕНДЕР: предотвращаем моргание в дополнительной функции рендера
    let newHtml = '';
    if (items.length === 0) {
        newHtml = '<p class="empty-msg">Товары не найдены</p>';
    } else {
        newHtml = items.map(product => typeof createProductCard === 'function' ? createProductCard(product) : '').join('');
    }
