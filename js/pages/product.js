...
let currentGalleryIndex = 0;
    ...
    window.updateProductUI = function() {
        ...
        // --- ОСЬ ТУТ МИ ПРАВИЛЬНО ЗЧИТУЄМО ВАГУ ДЛЯ ПОТОЧНОГО РОЗМІРУ ---
        ...
        // --- ОНОВЛЕННЯ ХАРАКТЕРИСТИКИ ВАГИ ТА ЇЇ РЯДКА ---
        ...
        // ... (далі решта коду функції без змін)
        ...
        const setupAddBtn = (id) => {
            ...
            if(isOutOfStock) {
                ...
                btn.innerHTML = `<span class="uppercase font-bold tracking-widest text-[11px]">Нема в наявності</span>`;
            } else if(isOnOrder) {
                btn.disabled = false;
                btn.classList.remove('opacity-50', 'cursor-not-allowed', 'bg-gray-400');
                btn.innerHTML = `<span class="uppercase font-bold tracking-widest text-[11px]">Замовити за ${formatPrice(currentPrice)}</span>`;
