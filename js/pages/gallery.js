...
// Предполагается, что клиент supabase импортируется или доступен в scope
...
async function loadGalleryMainFilters() {
    ...
    try {
        ...
        const { data: categories, error } = await supabase
            .from('categories')
            .select('id, name_uk')
            .order('id', { ascending: true }); // Если нужно задать порядок

        if (error) throw error;

        // 2. Базовая кнопка "Всі" с твоими Tailwind-классами
        let filtersHTML = `
            <a href="#all" class="gal-filter-btn px-6 py-2 border border-[var(--border)] rounded-full text-xs font-bold uppercase tracking-widest transition-all active" data-cat="all">Всі</a>
        `;

        // 3. Динамически добавляем остальные категории
        categories.forEach(cat => {
            filtersHTML += `
                <a href="#${cat.id}" class="gal-filter-btn px-6 py-2 border border-[var(--border)] rounded-full text-xs font-bold uppercase tracking-widest transition-all" data-cat="${cat.id}">${cat.name_uk}</a>
            `;
        });

        // 4. Отрисовываем фильтры
        filtersContainer.innerHTML = filtersHTML;

        // 5. Инициализируем логику переключения (если она не прописана отдельно)
        initGalleryFilterLogic();

    } catch (error) {
        console.error('Ошибка загрузки фильтров галереи:', error);
    }
}

function initGalleryFilterLogic() {
    const filterButtons = document.querySelectorAll('.gal-filter-btn');
    const galleryItems = document.querySelectorAll('.gallery-item'); // Убедись, что карточки фото имеют этот класс

    filterButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            e.preventDefault(); // Отменяем стандартный переход по якорю
            
            // Визуальное переключение активной кнопки (добавь свои классы для active состояния, если нужно)
            filterButtons.forEach(btn => btn.classList.remove('active', 'bg-[var(--gold-muted)]', 'text-white'));
            const targetBtn = e.currentTarget;
            targetBtn.classList.add('active', 'bg-[var(--gold-muted)]', 'text-white'); // Пример стилизации активной кнопки

            const filterValue = targetBtn.getAttribute('data-cat');

            // Фильтрация DOM-элементов сетки
            galleryItems.forEach(item => {
                // Предполагается, что на карточках галереи висит атрибут data-category="id_категории"
...
document.addEventListener('DOMContentLoaded', loadGalleryMainFilters);
